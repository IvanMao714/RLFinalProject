...
def set_sumo(gui, sumocfg_file_name, max_steps):
    ...
    sumo_cmd = [sumoBinary, "-c", os.path.join('envir/intersection', sumocfg_file_name), "--no-step-log", "true",
                "--waiting-time-memory", str(max_steps)]

    return sumo_cmd

def set_train_path(models_path_name, model_type):
    """
    Create a new model path with an incremental integer, also considering previously created model paths
    """
    models_path = os.path.join(os.getcwd(), models_path_name, model_type, '')
    ...
    data_path = os.path.join(models_path, model_type+'_'+new_version, '')
